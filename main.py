...
callback_check = tf.keras.callbacks.ModelCheckpoint(filepath=checkpoint_path,monitor='val_accuracy',verbose=1,save_freq='epoch',save_best_only=True,save_weights_only=False,mode='max')
...
model_history = model.fit(x_train,y_train,batch_size=32,epochs=12,callbacks=callback_check,validation_split=0.2)  # overfitting on the 10th epoch meaning that it reaches the optimal weights
